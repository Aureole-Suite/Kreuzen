		...
		loop {
			...
			if byte >= 0x20 {
				...
				let c = match byte {
					...
					0x10 => DialogueControl::Item(f.u16()?.into()),
					0x11 => DialogueControl::Voiceline(f.u32()?),
		...
		for part in &self.0 {
			match part {
				...
				DialoguePart::Control(c) => match c {
					...
					DialogueControl::Item(v) => { f.u8(0x10); f.u16(v.0); },
					DialogueControl::Voiceline(v) => { f.u8(0x11); f.u32(*v); },
