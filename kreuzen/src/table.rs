...
pub fn read_algo_table(f: &mut VReader) -> Result<Vec<Algo>, gospel::read::Error> {
	let mut table = Vec::new();
	while !f.remaining().is_empty() {
		let id = f.u16()?;
		let cond = f.u8()?;
...
pub fn read_anime_clip_table(f: &mut VReader) -> Result<Vec<AnimeClip>, AnimeClipError> {
	let mut table = Vec::new();
	loop {
		let kind = f.u32()?;
		if kind == 0 {
